          ...
          <section className="space-y-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">{TEACHER_TOOLS_CATALOG.length} tools in the shared catalog. Request and token totals are not recorded.</p>
              </div>
              <button
                type="button"
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-800/80 bg-[#0c1220]">
              <table className="w-full text-left text-sm">
                <thead className="border-b border-slate-800 bg-[#080d19] text-xs uppercase text-slate-400">
                  <tr>
                  ...
                  {tools.map((tool) => (
                    <tr key={tool.id}>
                      <td className="p-4 font-bold text-white">
                        {tool.title}
                        {tool.staged ? <span className="ml-2 text-xs text-amber-300">Staged</span> : null}
                      </td>
                      <td className="p-4 text-cyan-300">{tool.requiredPlan}</td>
                      <td className="p-4">—</td>
                      <td className="p-4">—</td>
                      <td className="p-4">
                        <button
                          ...
                          className="inline-flex cursor-pointer appearance-none items-center gap-1.5 rounded-md border border-transparent bg-transparent text-xs font-extrabold text-emerald-400"
                        >
                          {tool.active ? <CheckCircle2 className="h-3.5 w-3.5" /> : <XCircle className="h-3.5 w-3.5 text-slate-400" />}
